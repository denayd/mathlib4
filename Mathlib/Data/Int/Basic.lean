...
lemma negSucc_ofNat (n : ℕ) : -[1+ n] = -ofNat (n + 1) := rfl
...
lemma ofNat_add_negSucc (m n : ℕ) :
    ofNat m + -[1+ n] = subNatNat m (succ n) := rfl
lemma negSucc_add_ofNat (m n : ℕ) :
    -[1+ m] + ofNat n = subNatNat n (succ m) := rfl
lemma negSucc_add_negSucc (m n : ℕ) :
    -[1+ m] + -[1+ n] = -[1+ succ (m + n)] := rfl

lemma ofNat_mul_ofNat (m n : ℕ) :
    ofNat m * ofNat n = ofNat (m * n) := rfl
lemma ofNat_mul_negSucc (m n : ℕ) :
    ofNat m * -[1+ n] = negOfNat (m * succ n) := rfl
lemma negSucc_mul_ofNat (m n : ℕ) :
    -[1+ m] * ofNat n = negOfNat (succ m * n) := rfl
lemma negSucc_mul_negSucc (m n : ℕ) :
    -[1+ m] * -[1+ n] = ofNat (succ m * succ n) := rfl

attribute [local simp] neg_negSucc neg_ofNat_zero neg_ofNat_succ
...
/- # some properties of basic functions -/

protected lemma coe_nat_inj {m n : ℕ} (h : (↑m : ℤ) = ↑n) : m = n := Int.ofNat.inj h
...
ofNat_eq_ofNat_iff m n

lemma negSucc_inj_iff {m n : ℕ} : negSucc m = negSucc n ↔ m = n :=
⟨Int.negSucc.inj, λ H => by simp [H]⟩

lemma negSucc_eq_neg_succ_ofNat (n : ℕ) : -[1+ n] = -((ofNat n) + 1) := rfl
...
lemma natAbs_of_nat (n : ℕ) : natAbs (ofNat n) = n := rfl

lemma eq_zero_of_natAbs_eq_zero : ∀ {a : ℤ}, natAbs a = 0 → a = 0
| (ofNat _), H => congr_arg ofNat H
| -[1+ _],   H => absurd H (succ_ne_zero _)

lemma natAbs_pos_of_ne_zero {a : ℤ} (h : a ≠ 0) : 0 < natAbs a :=
(eq_zero_or_pos _).resolve_left $ mt eq_zero_of_natAbs_eq_zero h

lemma natAbs_zero : natAbs (0 : ℤ) = (0 : ℕ) := rfl

lemma natAbs_one : natAbs (1 : ℤ) = (1 : ℕ) := rfl

lemma natAbs_mul_self : ∀ {a : ℤ}, ofNat (natAbs a * natAbs a) = a * a
| (ofNat _) => rfl
| -[1+ _]   => rfl
...
by
  cases (Nat.lt_or_le n k) with
  | inl h' =>
    rw [subNatNat_of_lt h', ofNat_add_negSucc, succ_pred_eq_of_pos (Nat.sub_pos_of_lt h')]
    apply Eq.trans
  ...
  | inr h' =>
    ...
    rw [subNatNat_of_le h₂, ←ofNat_add, ofNat_eq_ofNat_iff, Nat.add_sub_assoc h']

lemma subNatNat_add_negSucc (m n k : ℕ) :
...
lemma negOfNat_mul_ofNat (m n : ℕ) : negOfNat m * ofNat n = negOfNat (m * n) := by
    rw [Int.mul_comm, ofNat_mul_negOfNat, Nat.mul_comm]

lemma negSucc_mul_negOfNat (m : ℕ) :
  ∀ n, -[1+ m] * negOfNat n = ofNat (succ m * n)
| 0        => rfl
| (succ n) => rfl

lemma negOfNat_mul_negSucc (m n : ℕ) :
...
by
  ...
  | succ m =>
    ...
    | inl h =>
      ...
      rw [← neg_ofNat_succ, Nat.mul_sub_left_distrib]
      rw [← succ_pred_eq_of_pos (Nat.sub_pos_of_lt h')]
      rfl
    | inr h =>
...
lemma negSucc_mul_subNatNat (m n k : ℕ) :
...
protected lemma subNatNat_eq_sub_ofNat {m n : ℕ} : subNatNat m n = ofNat m - ofNat n :=
subNatNat_elim m n (λ m n i => i = ofNat m - ofNat n)
  (λ i n => by
    simp only [Int.ofNat_add, Int.sub_eq_add_neg, Int.add_left_comm,
      Int.add_assoc, Int.add_right_neg]
    rfl)
  (λ i n => by
    simp only []
    rw [Int.sub_eq_add_neg, Int.ofNat_add, Int.ofNat_add, Int.ofNat_one,
        Int.negSucc_eq_neg_succ_ofNat, Int.neg_add, Int.neg_add, Int.neg_add,
        ← Int.add_assoc, ← Int.add_assoc, Int.add_right_neg, Int.zero_add])

theorem toNat_sub (m n : ℕ) : toNat (m - n) = m - n := rfl
